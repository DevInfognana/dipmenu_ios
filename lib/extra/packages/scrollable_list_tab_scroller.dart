...
class ScrollableListTabScrollerView extends StatefulWidget {
  final int itemCount;
  ...
  const ScrollableListTabScrollerView({
    super.key,
  ...
  @override
  ScrollableListTabScrollerViewState createState() =>
      ScrollableListTabScrollerViewState();
}

class ScrollableListTabScrollerViewState
    extends State<ScrollableListTabScrollerView> {
  ...
  Widget buildAddWidget(
      {required BuildContext context, required Widget child}) {
    return SizedBox(height: getDeviceType() == 'phone' ? 4.h:6.h, child: child);
  }
